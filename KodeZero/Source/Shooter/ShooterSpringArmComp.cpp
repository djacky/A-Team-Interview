...
void UShooterSpringArmComp::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
	...
	if (bDoLocationLag)
	{
		if (bUseCameraLagSubstepping && DeltaTime > CameraLagMaxTimeStep && CameraLagSpeed > 0.f)
		{
			...
			while (RemainingTime > KINDA_SMALL_NUMBER)
			{
				...
				const FVector TargetDesiredLoc = FMath::VInterpTo(PreviousDesiredLoc, LerpTarget, LerpAmount, CameraLagSpeed);

				DesiredLoc = FVector(bEnableCameraLagAxisX ? TargetDesiredLoc.X : LerpTarget.X,
					bEnableCameraLagAxisY ? TargetDesiredLoc.Y : LerpTarget.Y,
					bEnableCameraLagAxisZ ? TargetDesiredLoc.Z : LerpTarget.Z);
		...
		{
			const FVector TargetDesiredLoc = FMath::VInterpTo(PreviousDesiredLoc, DesiredLoc, DeltaTime, CameraLagSpeed);

			DesiredLoc = FVector(bEnableCameraLagAxisX ? TargetDesiredLoc.X : DesiredLoc.X,
				bEnableCameraLagAxisY ? TargetDesiredLoc.Y : DesiredLoc.Y,
				bEnableCameraLagAxisZ ? TargetDesiredLoc.Z : DesiredLoc.Z);
		}
